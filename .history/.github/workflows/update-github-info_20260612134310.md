...
description: Fetch recent GitHub Blog, Changelog, and Awesome Copilot workflow updates and open a reviewable PR for Mona.
on:
  workflow_dispatch:
  schedule: daily
engine: copilot
...
5. Keep the homepage source links aligned with the sources used in the update.
6. Use the `edit` tool to make the file changes.
7. Use `safe-outputs.create-pull-request` to open a pull request for Mona to review.
8. Do not write directly to `main` or push changes outside the safe-outputs PR flow.
